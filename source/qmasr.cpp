...
PQMASR_RECOG_RESULT CQmASRRecog::DoRecognize(vector<vector<float>> & feats, QMASR_MODE Mode)
{

	PQMASR_RECOG_RESULT pResult= new QMASR_RECOG_RESULT;
	...
	if (Mode == QASRM_CTC_GREEDY_SEARCH) //ctc greedy search
	{
		if (beam_size != 1)
		{
	...
	if (Mode == QASRM_CTC_RPEFIX_BEAM_SEARCH || Mode == QASRM_ATTENSION_RESCORING)
	{
		...
		if (Mode == QASRM_CTC_RPEFIX_BEAM_SEARCH)
		{
		...
		if (Mode == QASRM_ATTENSION_RESCORING)
		{
			int max_len = 0;
